...
# MSU Orion
HOMEhafs=/work/noaa/gmtb/${USER}/_feature_hwrf_physics/HAFS
...
#${PYTHON3} ./run_hafs.py -t ${dev} 2019090118 05L HISTORY \
#    config.EXPT=${EXPT} config.SUBEXPT=${EXPT}_regional_static_DTC_hwrf \
#    config.ictype=gfsnemsio config.bctype=gfsgrib2_0p25 \
#    config.NHRS=12 ${scrubopt} \
#    ../parm/hafs_regional_static_DTC_hwrf.conf

${PYTHON3} ./run_hafs.py -t ${dev} 2019090118 05L HISTORY \
    config.EXPT=${EXPT} config.SUBEXPT=${EXPT}_regional_static_DTC_hafsv0p \
    config.ictype=gfsnemsio config.bctype=gfsgrib2_0p25 \
    config.NHRS=12 ${scrubopt} \
    ../parm/hafs_regional_static_DTC_hafsv0p.conf    
